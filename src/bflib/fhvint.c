...
FHVINT *fhvint_create(void) {     /* create interface to FHV-factorization */
    FHVINT *fi;
    fi = talloc(1, FHVINT);
...
int fhvint_factorize(FHVINT *fi, int n, int (*col)(void *info, int j,
                                                   int ind[], double val[]),
                     void *info) {     /* compute FHV-factorization of specified matrix A */
...
int fhvint_update(FHVINT *fi, int j, int len, const int ind[],
                  const double val[]) {     /* update FHV-factorization after replacing j-th column of A */
    SGF *sgf = fi->lufi->sgf;
...
void fhvint_ftran(FHVINT *fi, double x[]) {     /* solve system A * x = b */
    FHV *fhv = &fi->fhv;
    LUF *luf = fhv->luf;
    int n = luf->n;
...
void fhvint_btran(FHVINT *fi, double x[]) {     /* solve system A'* x = b */
    FHV *fhv = &fi->fhv;
    LUF *luf = fhv->luf;
    int n = luf->n;
    int *pp_ind = luf->pp_ind;
...
double fhvint_estimate(FHVINT *fi) {     /* estimate 1-norm of inv(A) */
    double norm;
    xassert(fi->valid);
...
void fhvint_delete(FHVINT *fi) {     /* delete interface to FHV-factorization */
