...
static int set_penalty(struct csa *csa, double tol, double tol1) {
    ...
    for (i = 1; i <= m; i++) {
        ...
        if ((t = l[k]) != -DBL_MAX) {
            ...
            if (beta[i] < t - eps) {  /* lower bound is violated */
                c[k] = -1.0, count++;
            }
        }
        /* check upper bound */
        if ((t = u[k]) != +DBL_MAX) {
            eps = tol + tol1 * (t >= 0.0 ? +t : -t);
            if (beta[i] > t + eps) {  /* upper bound is violated */
                c[k] = +1.0, count++;
            }
        }
    }
...
tol1) {
    ...
    for (i = 1; i <= m; i++) {
        ...
        if (phase == 1 && c[k] < 0.0) {  /* -inf < x[k] <= l[k] */
            lk = -DBL_MAX, uk = l[k];
            orig = 0; /* artificial bounds */
        } else if (phase == 1 && c[k] > 0.0) {  /* u[k] <= x[k] < +inf */
            lk = u[k], uk = +DBL_MAX;
            orig = 0; /* artificial bounds */
        } else {  /* l[k] <= x[k] <= u[k] */
            lk = l[k], uk = u[k];
        ...
        if (lk != -DBL_MAX) {
            ...
            if (beta[i] < lk - eps) {  /* actual lower bound is violated */
                if (orig) {
                    ret = 2;
                    break;
                }
        ...
        if (uk != +DBL_MAX) {
            ...
            if (beta[i] > uk + eps) {  /* actual upper bound is violated */
                if (orig) {
                    ret = 2;
                    break;
                }
...
ind[/*1+num*/], double tol, double tol1) {
    ...
    for (t = 1; t <= num; t++) {
        ...
        xassert(1 <= i && i <= m);
        k = head[i]; /* x[k] = xB[i] */
        if (c[k] < 0.0) {  /* x[k] violates its original lower bound */
            lk = l[k];
            xassert(lk != -DBL_MAX);
            eps = tol + tol1 * (lk >= 0.0 ? +lk : -lk);
            if (beta[i] >= lk - eps) {  /* however, violation is close to zero */
                c[k] = 0.0, cnt++;
            }
        } else if (c[k] > 0.0) {  /* x[k] violates its original upper bound */
            uk = u[k];
            xassert(uk != +DBL_MAX);
            eps = tol + tol1 * (uk >= 0.0 ? +uk : -uk);
            if (beta[i] <= uk + eps) {  /* however, violation is close to zero */
                c[k] = 0.0, cnt++;
            }
        }
    }
...
static int choose_pivot(struct csa *csa) {
    ...
    if (csa->phase == 1 && csa->r_test == GLP_RT_FLIP && try <= 2) {  /* long-step ratio test */
        ...
        while (num < nbp) {  /* select and process a new portion of break points */
        ...
        if (t_best == 1) {  /* the very first break point was chosen; it is reasonable
        ...
        if (bp[t_best].i == 0) {  /* xN[q] goes to its opposite bound */
            csa->p = -1;
            csa->p_flag = 0;
            best_ratio = 1.0;
        } else if (bp[t_best].i > 0) {  /* xB[p] leaves the basis and goes to its lower bound */
            csa->p = +bp[t_best].i;
            xassert(1 <= csa->p && csa->p <= m);
            csa->p_flag = 0;
            best_ratio = fabs(tcol[csa->p]) / big;
        } else {  /* xB[p] leaves the basis and goes to its upper bound */
            csa->p = -bp[t_best].i;
            xassert(1 <= csa->p && csa->p <= m);
            csa->p_flag = 1;
            best_ratio = fabs(tcol[csa->p]) / big;
    ...
    } else {  /* Harris' two-pass ratio test */
    ...
    if (ret) {  /* invalidate dual basic solution components */
        ...
        for (t = 1; t < t_best; t++) {
            int i = (bp[t].i >= 0 ? bp[t].i : -bp[t].i);
            xassert(0 <= i && i <= m);
            if (i == 0) {  /* xN[q] crosses its opposite bound */
                xassert(1 <= csa->q && csa->q <= n - m);
                k = head[m + csa->q];
            } else {  /* xB[i] crosses its (lower or upper) bound */
                k = head[i]; /* x[k] = xB[i] */
            }
            c[k] += bp[t].dc;
...
static void play_bounds(struct csa *csa, int all) {
    ...
    for (i = 1; i <= m; i++) {
        if (all || tcol[i] != 0.0) {  /* beta[i] has changed in the adjacent basis */
            k = head[i]; /* x[k] = xB[i] */
            if (csa->phase == 1 && c[k] < 0.0) {  /* -inf < xB[i] <= lB[i] (artificial bounds) */
                if (beta[i] < l[k] - 1e-9)
                    continue;
            ...
            if (csa->phase == 1 && c[k] > 0.0) {  /* uB[i] <= xB[i] < +inf (artificial bounds) */
                if (beta[i] > u[k] + 1e-9)
                    continue;
                /* restore actual bounds */
                c[k] = 0.0;
...
static void display(struct csa *csa, int spec) {
    ...
    if (csa->inv_cnt) {  /* number of basis factorizations performed */
...
static int primal_simplex(struct csa *csa) {     /* primal simplex method main logic routine */
    SPXLP *lp = csa->lp;
    int m = lp->m;
    int n = lp->n;
    double *c = lp->c;
    ...
    if (!csa->beta_st) {
        ...
        if (!csa->phase) {
            if (set_penalty(csa, 0.97 * tol_bnd, 0.97 * tol_bnd1)) {  /* current basic solution is primal infeasible */
                /* start to minimize the sum of infeasibilities */
                csa->phase = 1;
            } else {  /* current basic solution is primal feasible */
    ...
    if (csa->num == 0) {
        ...
        switch (csa->phase) {
            case 1:
                ...
                if (!check_feas(csa, 2, tol_bnd, tol_bnd1)) {  /* feasible solution found; switch to phase II */
                    memcpy(c, csa->orig_c, (1 + n) * sizeof(double));
                    csa->phase = 2;
                    csa->d_st = 0;
                    goto loop;
    ...
    if (csa->p > 0) {
        ...
        if (lp->l[k] != lp->u[k]) {
            if (csa->p_flag) {  /* xB[p] goes to its upper bound */
                xassert(lp->u[k] != +DBL_MAX);
                if (fabs(beta[csa->p] - lp->u[k]) >= 1e-6) {
                    csa->degen = 0;
                    goto skip1;
                }
            } else if (lp->l[k] == -DBL_MAX) {  /* unusual case */
                goto skip1;
            } else {  /* xB[p] goes to its lower bound */
    ...
    if (se != NULL) {
        ...
        } else {  /* too many updates; reset reference space */
            se->valid = 0;
        }
    }
    ...
    if (perturb <= 0) {
        if (csa->phase == 1) {
            ...
            cnt = adjust_penalty(csa, csa->tcol.nnz, csa->tcol.ind,
                                 0.99 * tol_bnd, 0.99 * tol_bnd1);
            if (cnt) {  /* some coefficients were changed, so invalidate reduced
