...
static void trivial_lp(glp_prob *P, const glp_smcp *parm) {     /* solve trivial LP which has empty constraint matrix */
    GLPROW *row;
    GLPCOL *col;
    int i, j;
    ...
    for (i = 1; i <= P->m; i++) {
        ...
        if (row->type == GLP_LO || row->type == GLP_DB ||
            row->type == GLP_FX) {  /* row has lower bound */
        ...
        if (row->type == GLP_UP || row->type == GLP_DB ||
            row->type == GLP_FX) {  /* row has upper bound */
    ...
    for (j = 1; j <= P->n; j++) {
        ...
        if (col->type == GLP_FR)
            ...
            lo:
            col->stat = GLP_NL, col->prim = col->lb;
        else if (col->type == GLP_UP)
            up:
            col->stat = GLP_NU, col->prim = col->ub;
        ...
        if (col->type == GLP_FR || col->type == GLP_LO) {  /* column has no upper bound */
        ...
        if (col->type == GLP_FR || col->type == GLP_UP) {  /* column has no lower bound */
            if (zeta * col->dual > +parm->tol_dj) {
                P->dbs_stat = GLP_NOFEAS;
                if (P->some == 0 && parm->meth == GLP_PRIMAL)
                    P->some = P->m + j;
...
static int solve_lp(glp_prob *P, const glp_smcp *parm) {     /* solve LP directly without using the preprocessor */
    int ret;
    if (!glp_bf_exists(P)) {
        ret = glp_factorize(P);
        if (ret == 0);
...
static int preprocess_and_solve_lp(glp_prob *P, const glp_smcp *parm) {     /* solve LP using the preprocessor */
    NPP *npp;
    glp_prob *lp = NULL;
    glp_bfcp bfcp;
    int ret;
...
int glp_simplex(glp_prob *P, const glp_smcp *parm) {     /* solve LP problem with the simplex method */
    glp_smcp _parm;
    int i, j, ret;
...
double glp_get_obj_val(glp_prob *lp) {     /*struct LPXCPS *cps = lp->cps;*/
    double z;
    z = lp->obj_val;
...
double glp_get_row_prim(glp_prob *lp, int i) {     /*struct LPXCPS *cps = lp->cps;*/
    double prim;
    if (!(1 <= i && i <= lp->m))
...
double glp_get_row_dual(glp_prob *lp, int i) {     /*struct LPXCPS *cps = lp->cps;*/
    double dual;
    if (!(1 <= i && i <= lp->m))
...
double glp_get_col_prim(glp_prob *lp, int j) {     /*struct LPXCPS *cps = lp->cps;*/
    double prim;
    if (!(1 <= j && j <= lp->n))
...
double glp_get_col_dual(glp_prob *lp, int j) {     /*struct LPXCPS *cps = lp->cps;*/
    double dual;
    if (!(1 <= j && j <= lp->n))
...
int glp_get_it_cnt(glp_prob *P) {     /* get simplex solver iteration count */
    return P->it_cnt;
}
...
void glp_set_it_cnt(glp_prob *P, int it_cnt) {     /* set simplex solver iteration count */
    P->it_cnt = it_cnt;
    return;
}
