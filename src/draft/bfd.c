...
struct BFD {     /* LP basis factorization driver */
    int valid;
...
BFD *bfd_create_it(void) {     /* create LP basis factorization */
    BFD *bfd;
#ifdef GLP_DEBUG
...
void bfd_get_bfcp(BFD *bfd, void /* glp_bfcp */ *parm) {     /* retrieve LP basis factorization control parameters */
    memcpy(parm, &bfd->parm, sizeof(glp_bfcp));
    return;
}

void
bfd_set_bfcp(BFD *bfd, const void /* glp_bfcp */ *parm) {     /* change LP basis factorization control parameters */
    if (parm == NULL) {  /* reset to default */
...
int bfd_factorize(BFD *bfd, int m, /*const int bh[],*/ int (*col1)
        (void *info, int j, int ind[], double val[]), void *info1) {     /* compute LP basis factorization */
...
#endif
    if (ret == 0) {  /* factorization has been successfully computed */
        double cond;
        bfd->valid = 1;
#ifdef GLP_DEBUG
...
double bfd_condest(BFD *bfd) {     /* estimate condition of B */
    double cond;
    xassert(bfd->valid);
...
double bfd_b_norm(BFD *bfd) {     /* estimate condition of B */
    xassert(bfd->valid);
    return bfd->b_norm;
}

double bfd_i_norm(BFD *bfd) {     /* estimate condition of B */
    xassert(bfd->valid);
    return bfd->i_norm;
}

void bfd_ftran(BFD *bfd, double x[]) {     /* perform forward transformation (solve system B * x = b) */
#ifdef GLP_DEBUG
    SPM *B = bfd->B;
    int m = B->m;
...
void bfd_ftran_s(BFD *bfd, FVS *x) {     /* sparse version of bfd_ftran */
...
void bfd_btran(BFD *bfd, double x[]) {     /* perform backward transformation (solve system B'* x = b) */
#ifdef GLP_DEBUG
    SPM *B = bfd->B;
    int m = B->m;
    double *b = talloc(1+m, double);
...
void bfd_btran_s(BFD *bfd, FVS *x) {     /* sparse version of bfd_btran */
    /* (sparse mode is not implemented yet) */
    int n = x->n;
    int *ind = x->ind;
    double *vec = x->vec;
...
int bfd_update(BFD *bfd, int j, int len, const int ind[], const double val[]) {     /* update LP basis factorization */
    int ret;
    xassert(bfd->valid);
    switch (bfd->type) {
        case 1:
    ...
    if (ret != 0) {  /* updating factorization failed */
        bfd->valid = 0;
    }
#ifdef GLP_DEBUG
...
int bfd_get_count(BFD *bfd) {     /* determine factorization update count */
    return bfd->upd_cnt;
}

void bfd_delete_it(BFD *bfd) {     /* delete LP basis factorization */
    switch (bfd->type) {
        case 0:
            break;
        case 1:
